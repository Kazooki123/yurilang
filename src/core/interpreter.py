...
from src.core.modules     import load_module
...
# ------------------------------------------------------------------------
# @couple (Unions) - like a real C/Rust union, every variant shares the same
# underlying bytes. Setting one field and reading another is "type confusion"
# and is only allowed inside a @forbidden (Unsafe) block.
# ------------------------------------------------------------------------

def _union_format(type_name):
    return {"int": "q", "float": "d", "bool": "q"}.get(type_name)


def _union_packs(type_name, value):
    fmt = _union_format(type_name)
    if fmt is None:
        return None
    try:
        if type_name == "float":
        ...
        if iv >= 0x8000000000000000:
            iv -= 0x1000000000000000
        return struct.pack("<q", iv)
    except (struct.error, ValueError, TypeError, OverflowError):
        return None


def _union_unpack(type_name, buf):
    fmt = _union_packs(type_name)
    if fmt is None or buf is None:
        return None
    try:
        value = struct.unpack(f"<{fmt}", buf)[0]
        return value != 0 if type_name == "bool" else value
    except struct.error:
        return None


def _build_union(type_name, body):
    fields_def = unions[type_name]
    parts      = [p.strip() for p in body.split(",") if p.strip()]

    if len(parts) != 1:
        raise YuriRuntimeError(
            f"\n💔 YuriLang Error - couple_overcrowded\n\n"
            f" | @couple '{type_name}' can only hold ONE variant at a time, got {len(parts)}."
             " | A @couple is exclusive - like her heart, only one love at a time.\n"
        )
    
    if ":" not in parts[0]:
        raise YuriRuntimeError(
            f"@couple '{type_name}' literal must look like: {{ field: value }}"
        )
    
    fname, fval = parts[0].split(":", 1)
    fname       = fname.strip()
    fval        = fval.strip()
    
    if fname not in fields_def:
        raise YuriRuntimeError(
            f"\n💔 YuriLang Error - unknown_variant\n\n"
            f" | '{fname}' is not a variant of @couple '{type_name}'.\n"
            f" | Available: {', '.join(fields_def.keys())}\n"
        )
        
    value = evaluate(fval)
    field_type = fields_def[fname]
    packed = _union_packs(field_type, value)
    
    instance = {
        "__union_type__": type_name,
        "__union_active__": fname,
        "__union_bytes__": packed,
    }
    if packed is None:
        instance["__union_raw__"] = value
        
    return instance


def _union_field_access(instance, field):
    type_name = instance["__union_type__"]
    fields_def = unions.get(type_name, {})
    
    if field not in fields_def:
        raise YuriRuntimeError(
            f"\n💔 YuriLang Error - unknown_variant",
            f" | '{field}' is not a variant of @couple '{type_name}'.\n"
            f" | Available: {', '.join(fields_def.keys())}\n"
        )
        
    if unsafe_depth == 0:
        raise YuriRuntimeError(
            f"\n💔 YuriLang Error - unguarded_couple\n\n"
            f" | '{type_name}.{field}' is a @couple - touching her directly is dangerous!\n"
            f" | Active variant right now: '{instance.get('__union_active__')}'.\n\n"
            f" |> Hint: Wrap the access in an unsafe block:\n"
             "           @forbidden:\n"
            f"               @echo \"value is {{your_var.{field}}}\"\n"
        )
        
    field_type = fields_def[field]
    buf        = instance.get("__union_bytes__")
    
    if buf is not None and _union_format(field_type):
        return _union_unpack(field_type, buf)
    
    if field == instance.get("__union_active__"):
        return instance.get("__union_raw__")
    
    raise YuriRuntimeError(
        f"\n💔 YuriLang Error - type_confusion\n\n"
        f" | Cannot reinterpret '{type_name}.{instance.get('__union_active__')}' as a '{field_type}'.\n"
        f" | '{field_type}' isn't a fixed-size type that can safely overlap memory.\n"
    )
...
def _call_c(name, raw_args):
    # do NOT copy paste this block in run node and evaluate
...
def run_node(node):
    ...
    # IMPORT
        ...
        if alias:
            ns = {}
            from src.core.modules import get_module_functions
    ...
    # UNIONS
        ...
        for child in node.children:
            if child.type == "typed_field":
                fname, ftype = child.value
                ...
                if ":" in raw:
                    ...
                    fields[raw.strip()] = "any"
                    
        if not fields:
            raise YuriRuntimeError(
                f"@couple '{name}' has no variants - give her something to hold."
            )
    ...
    # UNSAFE
        ...
        try:
            for child in node.children:
                result = run_node(child)
                if isinstance(result, (ReturnSignal, BreakSignal, ContinueSignal)):
                    return result
        finally:
            unsafe_depth -= 1
