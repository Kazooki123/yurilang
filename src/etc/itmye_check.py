...
import platform
import os
from src.types import crush_hints, YURI_TYPES
...
def check_mutability(variables, awakened, owned, shared_ptrs,
glances, reaches):
    results = []
...
def check_ownership(variables, owned, shared_ptrs,
functions, awakened):
    results = []
    issues  = []
...
def check_type_hints(variables, crush_hints):
    from src.crush import check_hint, type_name
...
def calculate_score(mut_results, own_results, own_issues,
mov_results, type_results, os_score):
    all_scores = []
...
def run_itmye(variables, awakened, owned, shared_ptrs,
glances, reaches, functions):
    ###############################
    #         ITMYE CHECKER       #
    ...
    os_color = GREEN if os_score >= 90 else \
        CYAN  if os_score >= 75 else \
        YELLOW if os_score >= 50 else RED
    ...
    final_score = calculate_score(
        mut_results, own_results, own_issues,
        mov_results, type_results, os_score
    )
