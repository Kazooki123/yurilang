...
NOTES = {
    YuriErrorKind.AWAKENED_REASSIGN:
        "@awakening is YuriLang's immutability — once she knows herself, she stays herself.",
    YuriErrorKind.DEVOTED_MOVED:
        "@devoted follows ownership semantics — values move, not copy.",
    YuriErrorKind.INFINITE_LOOP:
        "YuriLang limits @fate loops to 10,000 iterations by default for safety.",
    YuriErrorKind.STACK_OVERFLOW:
        "Deep recursion can be rewritten as @fate loops for better performance.",
    YuriErrorKind.TYPE_MISMATCH:
        "YuriLang is dynamically typed — types are checked at runtime, not compile time.",
    YuriErrorKind.MODULE_NOT_FOUND:
        "Modules live in store/ as plain .yuri files — you can write your own!",
}


class YuriError:
    def __init__(self, kind, message, line=None, col=None,
                 source_line=None, suggestion=None):
        self.kind        = kind
